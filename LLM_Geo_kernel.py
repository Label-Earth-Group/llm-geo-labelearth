...
class Solution():
    ...
    def prompt_for_assembly_program(self):
        ...
        assembly_requirement = '\n'.join([f"{idx + 1}. {line}" for idx, line in enumerate(constants.assembly_requirement(self.save_dir))])
    ...
    def yield_LLM_assembly_response(self, review=True):
        ...
        if review:
            self.yield_ask_LLM_to_review_assembly_code()
...
#         , graph=True
    ...
    def get_debug_prompt(self, exception, code):
        ...
        debug_requirement_str = '\n'.join([f"{idx + 1}. {line}" for idx, line in enumerate(constants.debug_requirement(self.save_dir))])
